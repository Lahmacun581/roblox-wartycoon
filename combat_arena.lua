--[[
    ╔═══════════════════════════════════════════════════════════╗
    ║            Combat Arena GUI v1.0                          ║
    ║     Advanced ESP | Hitbox | Modern UI                    ║
    ║                  by Lahmacun581                           ║
    ╚═══════════════════════════════════════════════════════════╝
]]

print("[Combat Arena] Loading GUI v1.0...")
...
if not PlayerGui then
    warn("[Combat Arena] PlayerGui not found!")
    PlayerGui = LocalPlayer
end

-- Global state
getgenv().CombatArena = getgenv().CombatArena or {
    Version = "1.0",
    Enabled = {},
    Connections = {},
    ESPObjects = {},
...
-- Cleanup old GUI
if PlayerGui:FindFirstChild("CombatArenaGUI") then
    PlayerGui:FindFirstChild("CombatArenaGUI"):Destroy()
end
...
Header.BackgroundColor3 = Color3.fromRGB(25, 20, 25)
...
HeaderStroke.Color = Color3.fromRGB(255, 120, 120)
...
MinimizeBtn.BackgroundColor3 = Color3.fromRGB(40, 35, 40)
MinimizeBtn.Text = "—"
...
TabBar.Name = "TabBar"
TabBar.Size = UDim2.new(0, 160, 1, -80)
TabBar.Position = UDim2.new(0, 12, 0, 70)
TabBar.BackgroundColor3 = Color3.fromRGB(20, 18, 22)
...
ContentContainer.Size = UDim2.new(1, -195, 1, -80)
...
local function createTab(name, icon, color)
    ...
    tabBtn.BackgroundColor3 = Color3.fromRGB(28, 25, 30)
    ...
    tabIcon.Position = UDim2.new(0, 10, 0.5, -16)
    ...
    tabLabel.Position = UDim2.new(0, 48, 0, 0)
    ...
    tabBtn.MouseButton1Click:Connect(function()
        for _, tab in pairs(tabs) do
            ...
            TweenService:Create(tab.button, TweenInfo.new(0.2), {
                BackgroundColor3 = Color3.fromRGB(28, 25, 30)
            }):Play()
        ...
        TweenService:Create(tabBtn, TweenInfo.new(0.2), {
            BackgroundColor3 = Color3.fromRGB(38, 35, 42)
        }):Play()
...
local function createToggle(parent, text, callback)
    ...
    frame.BackgroundColor3 = Color3.fromRGB(28, 25, 30)
    frame.Parent = parent
...
local function createButton(parent, text, callback)
    ...
    btn.BackgroundColor3 = Color3.fromRGB(38, 35, 42)
    btn.Text = text
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {
            BackgroundColor3 = Color3.fromRGB(48, 45, 55)
        }):Play()
    end)
...
local ESPTab = createTab("ESP", "👁️", Color3.fromRGB(150, 100, 255))
local PlayerTab = createTab("Player", "🏃", Color3.fromRGB(100, 150, 255))
local MiscTab = createTab("Misc", "⚙️", Color3.fromRGB(200, 200, 100))

-- Show first tab
if tabs["Combat"] then
    tabs["Combat"].content.Visible = true
    tabs["Combat"].button.BackgroundColor3 = Color3.fromRGB(38, 35, 42)
...
do
    getgenv().CombatArena.HitboxSize = 10
    getgenv().CombatArena.HitboxPart = "HumanoidRootPart"
    getgenv().CombatArena.HitboxTeamCheck = true
    
    local hitboxConnection = nil
    local frameCounter = 0
    
    createSlider(CombatTab, "📏 Hitbox Size", 5, 50, 10, function(value)
        getgenv().CombatArena.HitboxSize = value
    end)
    ...
    createButton(CombatTab, "🎯 Hitbox Part: HumanoidRootPart", function()
        ...
        getgenv().CombatArena.HitboxPart = hitboxParts[currentPartIndex]
    ...
    createToggle(CombatTab, "🎯 Hitbox Expander", function(enabled)
        ...
        if enabled then
            hitboxConnection = RunService.Heartbeat:Connect(function()
                ...
                local size = getgenv().CombatArena.HitboxSize or 10
                local partName = getgenv().CombatArena.HitboxPart or "HumanoidRootPart"
                local teamCheck = getgenv().CombatArena.HitboxTeamCheck
                ...
                for _, player in ipairs(Players:GetPlayers()) do
                    if player ~= LocalPlayer and player.Character then
                        ...
                        if part and part:IsA("BasePart") then
                            if not getgenv().CombatArena.HitboxCache[part] then
                                getgenv().CombatArena.HitboxCache[part] = {
            ...
            table.insert(getgenv().CombatArena.Connections, hitboxConnection)
        else
            ...
            for part, data in pairs(getgenv().CombatArena.HitboxCache) do
                if part and part.Parent then
                    part.Size = data.OriginalSize
                    part.Transparency = data.OriginalTransparency
                end
            end
            getgenv().CombatArena.HitboxCache = {}
        end
    end)
end

-- ===== ESP TAB: ADVANCED ESP SYSTEM =====
do
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == Enum.KeyCode.U then
        ...
        if mouseEnabled then
            ...
            print("[Combat Arena] Mouse: ENABLED")
        else
            UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter
            print("[Combat Arena] Mouse: DISABLED")
        end
    end
end)
...
print("[Combat Arena] GUI v1.0 loaded!")
print("[Combat Arena] Press T to toggle GUI")
print("[Combat Arena] Press U to toggle Mouse")
print("[Combat Arena] ESP optimized for 60+ FPS")
